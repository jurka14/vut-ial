...
**/
    ...
    else
    {
        do
        {
            where = tmp; //uchovám hodnotu where
            if(tmp->Cont > Content) //jdu doleva
            {
                tmp = tmp->LPtr;
                continue;
            }
            if(tmp->Cont < Content) //jdu doprava
            {
                tmp = tmp->RPtr;
                continue;
            }
    ...
    if(where == NULL) //prázdný strom
    {
        *RootPtr = newptr;
        return;
    }

    if(where->Cont > Content) //vlevo
    {
        where->LPtr = newptr;
        return;
    }
    where->RPtr = newptr; //vpravo
...
**/
    ...
    while (ptr != NULL)
    {
        SPushP(Stack, ptr); //uzel na zásobník
        BTWorkOut(ptr); //zpracuj uzel
...
**/
    ...
    {
        RootPtr = STopPopP(&stack);
        BTWorkOut(RootPtr); //zpracuj uzel
...
**/
    ...
    while (ptr != NULL)
    {
        SPushP(StackP, ptr); //uzel na zásobník
        SPushB(StackB, true); //uzel na zásobník
        ptr = ptr->LPtr; //posun doleva
    }
...
**/
    ...
    bool zleva;
    tStackP stack_p;
    SInitP(&stack_p); //zásobník ukazatelů

    tStackB stack_b;
    SInitB(&stack_b); //zásobník boolů

    Leftmost_Postorder(RootPtr, &stack_p, &stack_b);

    while(!SEmptyP(&stack_p))
    {

        RootPtr = STopPopP(&stack_p);

        zleva = STopPopB(&stack_b);

        if(zleva) //přichází zleva, půjde doprava (byl navštíven poprvé)
        {
            SPushP(&stack_p, RootPtr);//uložíme hodnotu na zásobník a označíme ho jako navštíveného
            SPushB(&stack_b, false);
            Leftmost_Postorder(RootPtr->RPtr, &stack_p, &stack_b);//zpracujeme pravý podstrom

            continue;
        }

        BTWorkOut(RootPtr); //už jednou byl navštíven, vracíme se, zpracujeme ho
...
**/
    ...
    do
    {
        if((*RootPtr) == NULL) //vezmu ze zásobníku
        {
            if(!SEmptyP(&s))
            {
                *RootPtr = STopPopP(&s);
        ...
        {
            if((*RootPtr)->RPtr != NULL) //pravý jde do zásobníku
            {
